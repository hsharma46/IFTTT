...
router.get("/moverobot", async (req, res) => {
  ...
  try {
    ...
    let _coordinates_collection = await db.collection(
      constant.COLLLECTIONS.COORDINATES
    );
    let _coordinates_data = await _coordinates_collection
      .find({ name: _pointName.toUpperCase() })
      .toArray();
    if (_coordinates_data.length != 0)
      amqp.createConnectionAndChannel({
        command: constant.ROBOT_COMMAND.MOVE,
        coordinates: [_coordinates_data[0].coordinate],
      });
    else _message = `No Point exists with name ${_pointName}`;
